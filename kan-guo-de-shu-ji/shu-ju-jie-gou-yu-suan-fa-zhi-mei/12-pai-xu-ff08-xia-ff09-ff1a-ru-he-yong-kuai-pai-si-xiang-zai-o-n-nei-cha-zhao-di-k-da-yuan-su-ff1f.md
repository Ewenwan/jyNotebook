...
通过这样一步一步分解推导，我们可以得到 T\(n\) = 2^k_T\(n/2^k\)+k_n。当 T\(n/2^k\)=T\(1\) 时，也就是 n/2^k=1，我们得到 k=log2n 。我们将 k 值代入上面的公式，得到 T\(n\)=C_n+n_log2n 。如果我们用大 O 标记法来表示的话，T\(n\) 就等于 O\(nlogn\)。所以归并排序的时间复杂度是 O\(nlogn\)。
