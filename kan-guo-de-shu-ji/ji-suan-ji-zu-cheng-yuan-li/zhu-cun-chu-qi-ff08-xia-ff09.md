...
```
3.RAM5的片选输入端CS总是处于低电平
假设芯片与译码器本身都是好的，片选端cs为低电平可能的情况有:
* 该片的片选端与R/W端错连或短路
* 该片的片选端与CPU的MERQ端错连或短路
* 该片的片选端与地线错连或短路

4.存储器只能寻址A13=1的地址空间(RAM1,RAM3,RAM5,RAM7)，其他RAM将永远访问不到
```

#### 4.23

![](/assets/js-13.2.4.3-9.png)

```
体内地址(13位)|体号(3位)

注意M/IO是高电平进行访存控制，所以需要一个非门
```
